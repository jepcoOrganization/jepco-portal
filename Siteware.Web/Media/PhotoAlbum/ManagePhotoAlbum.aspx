<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="ManagePhotoAlbum.aspx.cs" Inherits="Siteware.Web.Pages.ManagePhotoAlbum" Async="true" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="rightpanel">
        <ul class="breadcrumbs">
            ...
            <li>Photo Ablum List</li>
        </ul>

        <div class="pageheader">
            <div>
                <asp:LinkButton ID="BtnDelete2" runat="server" class="btn btn-primary" Text="Delete Photo Ablum" OnClick="Delete_Click"
                    Style="position: relative; float: right; top: 31px; color: white">
                    <span class="glyphicon glyphicon-remove" style="color:white;padding-right: 4%;"></span> Delete Photo Ablum
                </asp:LinkButton>
                ...
                <asp:LinkButton ID="btnAdd2" runat="server" class="btn btn-primary" Text="Add Photo Ablum" OnClick="btnAdd_Click"
                    Style="position: relative; float: right; top: 31px; right: 0.5%; color: white">
                    <span class="glyphicon glyphicon-plus" style="color:white;padding-right: 4%;"></span> Add Photo Ablum
                </asp:LinkButton>
            </div>
                                        ...
                                        <table id="mytbl" class="table table-striped table-bordered table-hover dataTables-example">
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <%--<th class="head0"> Ablum ID</th>--%>
                                                    <th class="head0">Ablum Cover</th>
                                                    <th class="head1">Ablum Title</th>
                                                    <th class="head2">Language</th>
                                                    <th class="head0">Ablum Order</th>
                                                ...
                                                <asp:ListView ID="lstPages" runat="server" OnItemDataBound="lstPages_ItemDataBound" OnItemCommand="lstPages_ItemCommand">
                                                    <ItemTemplate>
                                                        <tr class="gradeX">
                                                            ...
                                                            <td>
                                                                <asp:Image ID="lblCategory" Style="height: 50px; width: 80px;" runat="server" ImageUrl='<%# Bind("Image") %>'></asp:Image>
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td class="center">
                                                                <asp:Label ID="lblTaget" runat="server" Text='<%# Bind("AlbumOrder") %>'></asp:Label>
                                                            </td>
                                                            <td class="center">&nbsp;
                                    <asp:LinkButton ID="lnkEdit" runat="server" CommandName="ViewEdit" CommandArgument='<%# Eval("ID") %>' ToolTip="Edit Photo Ablum">
                                        <span class="glyphicon iconsweets-create"></span>
                                    </asp:LinkButton>
                                                                &nbsp;
                                    <asp:LinkButton ID="lnkDelete" runat="server" CommandName="DeleteItem" CommandArgument='<%# Eval("ID") %>' ToolTip="Delete Photo Ablum">
                                        <span class="glyphicon glyphicon-remove ColorDelete" style="position: relative;top: 3px;"></span>
                                        
                                    </asp:LinkButton>
                                                                &nbsp;
                                    <asp:LinkButton ID="lnkAblumDetails" runat="server" CommandName="AblumDetails" CommandArgument='<%# Eval("ID") %>' ToolTip="Photo Ablum Details">
        ...
        $(document).ready(function () {
            ...
            var table = $('.dataTables-example').DataTable({
                ...
                buttons: [
                    //{ extend: 'copy' },
                    //{ extend: 'csv' },
                    { extend: 'excel', title: 'Welcome Note', text: '<span class="glyphicon glyphicon-file" style="color:white;padding-right: 4%;"></span> Export to Excel', className: 'btn btn-success btn-Excel', titleAttr: 'Excel' },
                    { extend: 'pdf', title: 'Welcome Note', text: '<span class="glyphicon glyphicon-file" style="color:white;padding-right: 4%;"></span> Export to PDF', className: 'btn btn-success btn-PDF', titleAttr: 'PDF' },
                    {
                        extend: 'print', title: 'Welcome Note', text: '<span class="glyphicon glyphicon-file" style="color:white;padding-right: 4%;"></span> Print', className: 'btn btn-success btn-Print', titleAttr: 'Print',
            ...
            $.ajax({
                ...
                url: "ManagePhotoAlbum.aspx/BindDatatoDropdown",
                data: "{}",
                dataType: "json",
                async: true,
                success: function (data) {
                    ...
                    var html = $("#mytbl_wrapper .dataTables_filter").html();
                    $.each(data.d, function (key, value) {
                        $("#lang-search").append($("<option></option>").html(value.LangName));
                    });

                    var selectedVal = $("#lang-search").val();
                    table.columns('.head2').search(selectedVal).draw();

                    $('#lang-search').on('change', function () {
                        table.columns('.head2').search(this.value).draw();
                    });
                },
