<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ConnectionFees.ascx.cs" Inherits="Controls_ConnectionFees" %>
...
<script>
    ...
    $(document).ready(function () {
            ...
            success: function (data) {
                ...
                if (data.statusCode = "Success") {
                    ...
                    if ((arrayBill).length > 0) {
                        $.each(arrayBill, function (key, value) {
                            if (value.billNumber == 99991131) {
                                var htmlbox = "<li id = 'connectionFees' > <div class='form-check' ><strong  class='form-check-label'>رسوم وصل التيار </strong><span class='LTR'></span></div>";
                                var htmlbox2 = "<div class='form-check2'><strong >قيمة الفاتورة</strong><p>" + value.billAmount + "</p></div></li>";

                                var collect = htmlbox + htmlbox2;
                                ...
                                var htmlbox = "<li> <div class='form-check' ><strong  class='form-check-label'>رقم الفاتورة</strong><span class='LTR'>" + value.billNumber + "</span></div>";
                                var htmlbox2 = "<div class='form-check2'><strong >قيمة الفاتورة</strong><p>" + value.billAmount + "</p></div></li>";
        ...
        $("#button-payment").click(function (e) {
            ...
            $.ajax({
                url: APIUrl + 'PaymentOrderHeaders/ReturnPaymentDisconnectionSummary',
                type: 'POST',
                beforeSend: function (xhr) {
                    xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem("MiddlewareToken"));
                },
                data: JSON.stringify(
                    {
                        ...
                        PaymentSource: "0",
                        Language: "AR",
                        PaymentType: "PaymentDisconnection",
                        AllDocument: true,
                        PaymentGatewayId: "1",
