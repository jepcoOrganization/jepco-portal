...
<div class="greybg subscribe_div">
    <div class="step_form">
        <div class="subscribe_div1">
            <h3>معلومات الإشتراك</h3>
            <label>الرجاء ادخال البيانات المطلوبة لتعديل الإشتراك </label>
                            ...
                            <div class="row">
                             ...
                             <div class="col-lg-6 col-md-6 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number" placeholder="رقم المرجع" name="refNumber" id="refNumber" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength="13" disabled /> 
...
<script>    
    ...
    $(document).ready(function () {
        ...
        // Api Get FileNumber Details By File Number : 
        $.ajax({
            type: "POST",
            url: APIUrl + "CustomerInformationDetails/GetCustomerInformationDetailByFileNumber",
        ...
        if (arrayAddressAPI != undefined) {
            ...
            AreaIdLast = arrayAddressAPI.areaId;
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    BranchId: Number(ProvinceId),
                    AreaId: Number(AreaIdLast),
                    NeighborhoodId: Number(NeighborhoodIdLast),
                    LanguageId: "AR"
                }),
                contentType: "application/json; charset=utf-8",
                async: false,
            ...
            $("select.city-select").val(ProvinceId)
            $(".areas-select").val(AreaIdLast)
            $(".neighborhoods-select").val(NeighborhoodIdLast)
            if (StreetIdLast == "") {
                $(".street-select").val(0)
            } else {
                $(".street-select").val(StreetIdLast)
...
<%--        function step1Valid() {
    ...
    $(".submitAdd").click(function () {
        ...
        console.log("File : ", FileNumberAddressesDetail)

        $.ajax({
            type: "POST",
            url: APIUrl + "CustomerInformationDetails/UpdateCustomerInformationDetails ",
            beforeSend: function (xhr) {
                xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem("MiddlewareToken"));
            },
            data: JSON.stringify({
